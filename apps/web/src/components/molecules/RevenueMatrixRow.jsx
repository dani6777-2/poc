    ...
    <tr className="hover:bg-tx-primary/[0.02] transition-colors group h-20">
      <td className="p-6 font-black text-[13px] text-tx-primary sticky left-0 z-10 bg-secondary border-r border-border-base/40 shadow-2xl">
        <div className="flex items-center gap-5">
          <div
            className={`w-3 h-3 rounded-full transition-all ${isSaving ? "bg-accent animate-ping" : "bg-tx-primary/10 group-hover:bg-success/50 shadow-glow-success"}`}
          />
          <span className="group-hover:text-success transition-colors uppercase tracking-[0.15em] shrink-0">
            {row.source}
          </span>
          {isSaving && (
            <Badge
              variant="accent"
              size="sm"
              className="scale-90 origin-left font-black tracking-widest text-[8px]"
            >
              SYNC
            </Badge>
          )}
