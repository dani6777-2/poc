...
const InventoryItemRow = ({
  ...
  return (
    <tr className="hover:bg-tx-primary/[0.01] transition-colors group">
      ...
      {type === "block-a" ? (
        <td className="p-6 text-center font-black tabular-nums text-sm">
          {item.quantity || "0"}
        </td>
      ) : null}
        ...
        {type === "block-a" ? (
          item.prev_month_price ? (
            <div className="flex flex-col items-center gap-1">
              <span className="text-[9px] font-black text-tx-muted opacity-20">
                {fmt(item.prev_month_price)}
              </span>
              {diffA !== null && (
                <div
                  className={`text-[10px] font-black flex items-center gap-1 ${diffA > 0 ? "text-danger" : "text-success"}`}
                >
            ...
            <Badge variant="accent" size="sm" className="scale-75 opacity-30">
              NEW_NODE
            </Badge>
          )
        ) : hasDeltaB ? (
          <Badge
            ...
            className="px-4 tracking-tighter"
          >
            {deltaB > 0 ? "▲ INC" : deltaB < 0 ? "▼ DEC" : "STABLE"}{" "}
            {fmt(Math.abs(deltaB))}
          </Badge>
        ) : (
          <span className="text-[10px] font-black text-tx-muted opacity-10 uppercase tracking-widest">
            UNTRACKED
          </span>
        )}
      </td>
      ...
      <td className="p-6 text-right pr-10">
        <div className="flex gap-2 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
          <Button
            ...
            size="sm"
            onClick={() => onEdit(item)}
            className="w-9 h-9 p-0 rounded-lg hover:bg-accent/10 hover:text-accent"
          >
            <svg
              className="w-4 h-4"
              fill="none"
          ...
          <Button
            ...
            className="w-9 h-9 p-0 rounded-lg hover:bg-danger/10 hover:text-danger"
          >
            <svg
              className="w-4 h-4"
              fill="none"
