...
const InventoryEditorModal = ({ 
  ...
  const protocol = type === 'block-a' ? 'INVENTORY_PROTOCOL_A' : 'PERISHABLE_PROTOCOL_B';
  const title = editing 
    ? (type === 'block-a' ? 'Edit Matrix Entry' : 'Edit Fresh Entry') 
    : (type === 'block-a' ? 'Manual Asset Registry' : 'New Market Ingestion');
      ...
      <Card className="w-full max-w-2xl p-10 md:p-14 relative z-10 animate-in zoom-in-95 duration-500 rounded-[3rem]">
        <div className="flex items-center justify-between mb-12">
           <div className="space-y-1">
              <h2 className="text-3xl font-black text-tx-primary uppercase tracking-tighter leading-none">{title}</h2>
              <Badge variant={isSuccess ? 'success' : 'accent'} className={`tracking-[0.4em] opacity-80 ${isSuccess ? 'text-primary' : ''}`}>{protocol}</Badge>
           </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <InventoryField label="Asset Descriptor / SKU" className="md:col-span-2">
              <Input name="name" value={form.name} onChange={handleChange} required placeholder={type === 'block-a' ? "e.g. Premium Basmati Evolution" : "e.g. Avocado Hass Selected"} />
            </InventoryField>
            ...
            <InventoryField label={type === 'block-a' ? "Acquisition Channel" : "Market Channel"}>
            ...
            <InventoryField label="Previous Reference ($)" className={type === 'block-b' ? 'md:col-span-2' : ''}>
              <Input name="prev_month_price" type="number" value={form.prev_month_price} onChange={handleChange} className="font-black tabular-nums opacity-60" placeholder={type === 'block-b' ? "Optional for delta tracking" : ""} />
            </InventoryField>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-5 mt-14">
            <Button type="submit" variant={isSuccess ? 'success' : 'accent'} className={`flex-1 py-7 uppercase tracking-[0.3em] ${isSuccess ? 'text-primary' : ''}`}>
              {editing ? 'Update Architecture' : (type === 'block-a' ? 'Commit New Asset' : 'Register produce')}
            </Button>
