  ...
  return (
    <div className="space-y-10 page-entry">
        ...
        <div className="p-4 lg:p-6 border-b border-border-base bg-tx-primary/[0.02] flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Badge
              variant="purple"
              glow
              className="px-5 py-1.5 font-black uppercase tracking-[0.3em] text-[9px]"
            >
              Prospective Analysis
            </Badge>
            <h3 className="text-[14px] font-black uppercase tracking-[0.4em] text-tx-primary">
              Consolidated Annual Capital Matrix
            </h3>
          </div>
          <Badge
            variant="muted"
            className="opacity-40 tracking-widest font-black text-[9px]"
          >
            IO_CORE_PLANNER
          </Badge>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[1400px]">
            <thead className="sticky top-0 z-20">
              <tr className="text-[10px] font-black uppercase tracking-[0.4em] text-tx-muted bg-secondary/95 backdrop-blur-xl border-b border-border-base/40">
                <th className="p-7 px-10 sticky left-0 z-30 bg-secondary shadow-2xl border-r border-border-base/40">
                  Liquidity Vectors
                </th>
                {MONTH_LABELS.map((m) => (
                  <th key={m} className="p-7 text-right bg-secondary/80">
                    {m}
                  </th>
                ))}
                <th className="p-7 text-right text-tx-primary bg-tx-primary/[0.04]">
                  Total Cap
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border-base/40">
              <tr className="hover:bg-success/[0.02] transition-colors group">
                <td className="p-6 px-10 font-black text-[13px] text-success sticky left-0 z-20 bg-secondary border-r border-border-base/40 uppercase tracking-widest leading-none">
                  (+) Actual Revenues
                  <div className="text-[8px] opacity-30 mt-2 font-black">
                    REVENUE_STREAM_V4
                  </div>
                </td>
                {totalRevenuesMonth.map((v, i) => (
                  <td
                    key={i}
                    className="p-6 text-right font-black text-sm text-success/60 tabular-nums tracking-tighter"
                  >
                    {v > 0 ? fmt(v) : "—"}
                  </td>
                ))}
                <td className="p-6 text-right font-black text-base text-success tabular-nums tracking-tighter bg-success/[0.02]">
                  {fmt(totalRevenuesAnnual)}
                </td>
              </tr>
              <tr className="hover:bg-warning/[0.02] transition-colors group">
                ...
                {totalPlannedMonth.map((v, i) => (
                  <td
                    key={i}
                    className="p-6 text-right font-black text-sm text-warning/60 tabular-nums tracking-tighter"
                  >
                    {v > 0 ? fmt(v) : "—"}
                  </td>
                ))}
                <td className="p-6 text-right font-black text-base text-warning tabular-nums tracking-tighter bg-warning/[0.02]">
                  {fmt(totalPlannedAnnual)}
                </td>
              </tr>
              <tr className="hover:bg-purple/[0.02] transition-colors group">
                ...
                {totalActualMonth.map((v, i) => (
                  <td
                    key={i}
                    className="p-6 text-right font-black text-sm text-purple/60 tabular-nums tracking-tighter"
                  >
                    {v > 0 ? fmt(v) : "—"}
                  </td>
                ))}
                <td className="p-6 text-right font-black text-base text-purple tabular-nums tracking-tighter bg-purple/[0.02]">
                  {fmt(totalActualAnnual)}
                </td>
              </tr>
      ...
      {sections.map((sec) => {
        ...
        const secTotals = MONTH_KEYS.map((m) =>
            ...
            >
              ...
              <div className="flex gap-6 md:p-12 items-center">
                <div className="hidden lg:flex flex-col items-end">
                  ...
                  <div className="flex items-center gap-4">
                    <strong className="text-xl font-black text-tx-primary tracking-tighter transition-all">
                      {fmt(secTotal)}
                    </strong>
                    {totalRevenuesAnnual > 0 && (
                      <Badge
                        variant="muted"
                        className="font-black uppercase text-[10px] tracking-widest"
                      >
                        {Math.round((secTotal / totalRevenuesAnnual) * 100)}%
                        REV
                      </Badge>
                    )}
                  </div>
                ...
                <table className="w-full text-left border-collapse min-w-[1400px]">
                  <thead>
                    <tr className="text-[10px] font-black uppercase text-tx-muted/40 tracking-[0.3em] bg-tx-primary/[0.02] border-b border-border-base/40">
                      <th className="p-6 px-10 sticky left-0 z-20 bg-secondary shadow-xl border-r border-border-base/40">
                        Expense Line
                      </th>
                      {MONTH_LABELS.map((m) => (
                        <th key={m} className="p-6 text-right tabular-nums">
                          {m}
                        </th>
                      ))}
                  ...
                  <tbody className="divide-y divide-border-base/40">
                    {secRows.map((row) => {
                      ...
                      return (
                        <tr
                          key={row.id}
                          className={`hover:bg-tx-primary/[0.02] transition-colors group h-16 ${auto ? "bg-accent/[0.02]" : ""}`}
                        >
                          <td className="p-3 px-10 text-[13px] font-black text-tx-primary sticky left-0 z-20 bg-secondary border-r border-border-base/40 shadow-2xl flex items-center gap-3">
                            <span className="uppercase tracking-widest group-hover:text-accent transition-colors truncate">
                              {row.description}
                            </span>
                            {auto && (
                              <Badge
                                variant="info"
                                className="scale-[0.6] origin-left font-black"
                              >
                                AUTO
                              </Badge>
                            )}
                          ...
                          {MONTH_KEYS.map((m) => (
                            <td key={m} className="p-1.5 px-3">
                              {auto ? (
                                <span className="block text-right text-[11px] font-bold text-tx-primary/30 py-3 tabular-nums tracking-tighter">
                                  {(parseFloat(row[m]) || 0) > 0
                                    ? fmt(parseFloat(row[m]))
                                    : "—"}
                                </span>
                              ) : (
                                <input
                                  ...
                                  value={row[m] || ""}
                                  onChange={(e) =>
                                    handleCellChange(row.id, m, e.target.value)
                                  }
                                  className="w-full h-10 bg-tx-primary/[0.01] hover:bg-tx-primary/[0.05] focus:bg-accent/5 focus:ring-1 focus:ring-accent/20 rounded-xl px-3 text-right text-[11px] font-bold text-tx-primary outline-none transition-all [appearance:textfield] tracking-tighter"
                                  placeholder="0"
                                />
                              )}
                            </td>
                          ))}
                          <td className="p-4 text-right font-black text-[13px] text-tx-primary bg-tx-primary/[0.04] tabular-nums tracking-tighter">
                  ...
                  <tfoot className="border-t border-border-base/40 bg-tx-primary/[0.06] text-[11px] font-black uppercase text-tx-secondary tracking-[0.2em] relative">
                    <tr className="h-20">
                      <td className="p-6 px-10 sticky left-0 z-20 bg-secondary shadow-2xl border-r border-border-base/40">
                        Aggregate Sum {sec.name}
                      </td>
                      {secTotals.map((t, i) => (
                        <td
                          key={i}
                          className="p-6 text-right tabular-nums tracking-tighter"
                        >
                          {fmt(t)}
                        </td>
                      ))}
                      <td className="p-6 text-right text-tx-primary tabular-nums tracking-tighter text-lg">
                        {fmt(secTotal)}
                      </td>
                      <td />
                    </tr>
